...
class _ChatAnimatedListState extends State<ChatAnimatedList>
  ...
  void initState() {
    ...
    if (widget.reversed) {
      ...
      debugPrint('⏭️ ChatAnimatedList: reversed=true，不需要初始滚动调整');
    } else {
  ...
  void _subsequentScrollToEnd(Message data) async {
    ...
    if (widget.shouldScrollToEndWhenSendingMessage == true &&
      ...
      // See https://stackoverflow.com/a/77175903 for more details.
      if (widget.reversed) {
        // In reversed mode, bottom is at offset 0, so we only need to scroll
  ...
  void _scrollToEnd(Message data) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // In reversed mode, the list naturally starts at the bottom (offset=0),
      // so we don't need to scroll for initial messages. Only scroll when
      // user sends a new message (if shouldScrollToEndWhenSendingMessage is true).
      if (widget.reversed) {
        // For reversed lists, only scroll if it's a user-sent message and
        // shouldScrollToEndWhenSendingMessage is enabled
  ...
  void _adjustInitialScrollPosition() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (_oldList.isEmpty) {
        debugPrint('⏭️ _adjustInitialScrollPosition: 跳过 - 列表为空');
        _needsInitialScrollPositionAdjustment = false;
        return;
      }

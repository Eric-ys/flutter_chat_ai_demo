...
class HiveChatController
    ...
    try {
      ...
      if (index != null) {
        ...
        for (int i = 0; i < currentMessages.length; i++) {
          final msgTime = currentMessages[i].createdAt?.millisecondsSinceEpoch ?? 0;
  ...
  List<Message> get messages {
    if (_cachedMessages != null) {
      debugPrint('💾 HiveChatController.messages: 使用缓存，${_cachedMessages!.length} 条消息');
    ...
    try {
      ...
      debugPrint('📦 从 Hive box 读取数据完成: ${boxValues.length} 条，耗时: ${boxValuesDuration.inMilliseconds}ms');
      ...
      _cachedMessages = boxValues
          ...
          .whereType<Message>()
          .toList();
      final deserializeDuration = DateTime.now().difference(deserializeStartTime);
      debugPrint('🔄 消息反序列化完成: ${_cachedMessages!.length} 条，耗时: ${deserializeDuration.inMilliseconds}ms');
      ...
      debugPrint('✅ HiveChatController: 成功加载 ${_cachedMessages!.length} 条消息，总耗时: ${totalDuration.inMilliseconds}ms');
    } catch (e) {
      final totalDuration = DateTime.now().difference(loadStartTime);
      debugPrint('❌ HiveChatController: 从 Hive 加载消息失败，耗时: ${totalDuration.inMilliseconds}ms, 错误: $e');
      _cachedMessages = [];
    }
